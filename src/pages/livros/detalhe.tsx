...
const DetalheLivro = () => {
    ...
    useEffect(()=>{
        ...
        const sql = `select * from livros were isbn = ${_isbn}`;
        send(sql);
    ...
    const send = (sql: string) => {
        sendAsync(sql).then((result:Livro) => {
            console.log(result)
            setLivro(result)
        });
    }
